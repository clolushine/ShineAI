...
public class GroqAISettingPanel implements Configurable, Disposable {
    ...
    private void init() {
        ...
        if (DBUtil.getLLMsByKey(GROQ_AI_KEY).isEmpty()) {
            updateModels();
        } else {
    ...
    public void reset() {
        ...
        JsonObject settingInfo = state.getAISettingInfo(GROQ_AI_CONTENT_NAME);
    ...
    public void apply() {
        ...
        state.setAISettingInfo(GROQ_AI_CONTENT_NAME,setInfo);
    ...
    private void createUIComponents() {
        ...
        TitledSeparator atBt = new TitledSeparator(MsgEntryBundle.message("ui.setting.server.groq.apikey.title"));
        apikeyTitledBorderBox.add(atBt,BorderLayout.CENTER);
    }

    public DefaultComboBoxModel<String> modelsToComboBoxModel() {

        JsonArray models = DBUtil.getLLMsByKey(GROQ_AI_KEY);
    ...
    public void updateModels() {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            assert modelsCombobox != null;
            JsonArray models = ShineAIUtil.getAIModels(GROQ_AI_KEY,GROQ_AI_LLM_API,refreshModelsButton);
            ...
            if (!models.isEmpty()) DBUtil.setLLMsByKey(GROQ_AI_KEY,models);
            modelsCombobox.setModel(modelsToComboBoxModel());
            JsonElement currentModel = stateStore.getAISettingInfoByKey(GROQ_AI_CONTENT_NAME,"aiModel");
    ...
    private void addNewApikey(JComponent _component) {
        ...
        apikeyList.add(new AIApikeyComponent(apikeyInfo,thisApiKeys,_component,GROQ_AI_CONTENT_NAME,false),0);
        thisApiKeys.add(0,apikeyInfo);
        // 这里需要写入state
        stateStore.setAISettingInfoByKey(GROQ_AI_CONTENT_NAME,"apiKeys", JsonUtil.getJsonArray(thisApiKeys));
        updateLayout();
        scrollTop();
    }
    ...
    public void initApiKeysPanel() {
        if (!thisApiKeys.isEmpty()) {
            for (JsonObject item : thisApiKeys) {
                AIApikeyComponent apiKeyItem = new AIApikeyComponent(item,thisApiKeys,apikeyList,GROQ_AI_CONTENT_NAME,false);
