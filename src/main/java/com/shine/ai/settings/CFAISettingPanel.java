...
import static com.shine.ai.MyToolWindowFactory.CLOUDFLARE_AI_CONTENT_NAME;
...
public class CFAISettingPanel implements Configurable, Disposable {
    ...
    public void reset() {
        ...
        enableStreamCheckBox.setSelected(state.CFEnableStream);
        streamSpeedField.setValue(state.CFStreamSpeed);
    ...
    public boolean isModified() {
        ...
        return !state.CFEnableStream == enableStreamCheckBox.isSelected() ||
                !(state.CFStreamSpeed == streamSpeedField.getValue()) ||
                !StringUtil.equals(state.CFCurrentModel, (String) modelsCombobox.getSelectedItem());
    }
    ...
    public void apply() {
        ...
        state.CFEnableStream = enableStreamCheckBox.isSelected();
        state.CFStreamSpeed = streamSpeedField.getValue();
        state.CFCurrentModel = (String) modelsCombobox.getSelectedItem();
    ...
    public String getDisplayName() {
        return MsgEntryBundle.message("ui.setting.server.cloudflare.key");
    }
    ...
    public void updateModels() {
        AIAssistantSettingsState state = AIAssistantSettingsState.getInstance();
        assert modelsCombobox != null;
        String[] models = ShineAIUtil.getAIModels(CLOUDFLARE_AI_CONTENT_NAME,refreshModelsButton);
        HashMap<String, String> cfModels = new HashMap<>();
        for (String model : models) {
            // 根据 model 获取对应的值，例如从另一个数组或其他数据源中获取
            cfModels.put(model,model);
        }
        int beforeLength = state.CFModels.size();
    ...
    public DefaultComboBoxModel<String> modelsToComboBoxModel() {
        ...
        state.CFModels.values().forEach(comboboxModels::addElement); // 将值添加到模型
