...
import static com.shine.ai.MyToolWindowFactory.Google_AI_CONTENT_NAME;
...
public class GoogleAISettingPanel implements  Configurable, Disposable {
    ...
    public void reset() {
        ...
        enableStreamCheckBox.setSelected(state.GOEnableStream);
        streamSpeedField.setValue(state.GOStreamSpeed);
    ...
    public boolean isModified() {
        ...
        return !state.GOEnableStream == enableStreamCheckBox.isSelected() ||
                !(state.GOStreamSpeed == streamSpeedField.getValue()) ||
                !StringUtil.equals(state.GOCurrentModel, (String) modelsCombobox.getSelectedItem());
    }
    ...
    public void apply() {
        ...
        state.GOEnableStream = enableStreamCheckBox.isSelected();
        state.GOStreamSpeed = streamSpeedField.getValue();
        state.GOCurrentModel = (String) modelsCombobox.getSelectedItem();
    ...
    public String getDisplayName() {
        return MsgEntryBundle.message("ui.setting.server.google.key");
    }
    ...
    public DefaultComboBoxModel<String> modelsToComboBoxModel() {
        ...
        state.GOModels.values().forEach(comboboxModels::addElement); // 将值添加到模型
    ...
    public void updateModels() {
        AIAssistantSettingsState state = AIAssistantSettingsState.getInstance();
        assert modelsCombobox != null;
        String[] models = ShineAIUtil.getAIModels(Google_AI_CONTENT_NAME,refreshModelsButton);
        HashMap<String, String> cfModels = new HashMap<>();
        for (String model : models) {
            // 根据 model 获取对应的值，例如从另一个数组或其他数据源中获取
            cfModels.put(model,model);
        }
        int beforeLength = state.GOModels.size();
