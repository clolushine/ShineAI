...
public class AnthropicAISettingPanel implements Configurable, Disposable {
    ...
    public AnthropicAISettingPanel() {
        createRefreshButton();
        createApikeyList();
        init();
    }
    ...
    private void init() {
        ...
        if (DBUtil.getLLMsByKey(Anthropic_AI_KEY).isEmpty()) {
            updateModels();
        } else {
    ...
    public void reset() {
        ...
        JsonObject settingInfo = state.getAISettingInfo(Anthropic_AI_CONTENT_NAME);
    ...
    public void apply() {
        ...
        state.setAISettingInfo(Anthropic_AI_CONTENT_NAME,setInfo);
    ...
    public String getDisplayName() {
        return Anthropic_AI_CONTENT_NAME;
    }
    ...
    private void createUIComponents() {
        ...
        TitledSeparator mdModel = new TitledSeparator(MsgEntryBundle.message("ui.setting.server.anthr.models.title"));
        modelTitledBorderBox.add(mdModel,BorderLayout.CENTER);

        apikeyTitledBorderBox = new JPanel(new BorderLayout());
        TitledSeparator atBt = new TitledSeparator(MsgEntryBundle.message("ui.setting.server.anthr.apikey.title"));
        apikeyTitledBorderBox.add(atBt,BorderLayout.CENTER);
    }

    public DefaultComboBoxModel<String> modelsToComboBoxModel() {
        JsonArray models = DBUtil.getLLMsByKey(Anthropic_AI_KEY);
    ...
    public void updateModels() {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            if (!models.isEmpty()) DBUtil.setLLMsByKey(Anthropic_AI_KEY,models);
            modelsCombobox.setModel(modelsToComboBoxModel());
            JsonElement currentModel = stateStore.getAISettingInfoByKey(Anthropic_AI_CONTENT_NAME,"aiModel");
    ...
    private void addNewApikey(JComponent _component) {
        ...
        apikeyList.add(new AIApikeyComponent(apikeyInfo,thisApiKeys,_component,Anthropic_AI_CONTENT_NAME,false),0);
        thisApiKeys.add(0,apikeyInfo);
        // 这里需要写入state
        stateStore.setAISettingInfoByKey(Anthropic_AI_CONTENT_NAME,"apiKeys", JsonUtil.getJsonArray(thisApiKeys));
        updateLayout();
        scrollTop();
    }
    ...
    public void initApiKeysPanel() {
        if (!thisApiKeys.isEmpty()) {
            for (JsonObject item : thisApiKeys) {
                AIApikeyComponent apiKeyItem = new AIApikeyComponent(item,thisApiKeys,apikeyList,Anthropic_AI_CONTENT_NAME,false);
