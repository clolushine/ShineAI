...
import static com.shine.ai.vendors.AIVendors.DeepSeek_AI_KEY;

public class NvidiaAISettingPanel implements Configurable, Disposable {
    ...
    private void createUIComponents() {
        ...
        TitledSeparator mdModel = new TitledSeparator(MsgEntryBundle.message("ui.setting.server.nvidia.models.title"));
        modelTitledBorderBox.add(mdModel,BorderLayout.CENTER);

        apikeyTitledBorderBox = new JPanel(new BorderLayout());
        TitledSeparator atBt = new TitledSeparator(MsgEntryBundle.message("ui.setting.server.nvidia.apikey.title"));
        apikeyTitledBorderBox.add(atBt,BorderLayout.CENTER);
    }

    public DefaultComboBoxModel<String> modelsToComboBoxModel() {

        JsonArray models = DBUtil.getLLMsByKey(Nvidia_AI_KEY);
    ...
    public void updateModels() {
        ...
        CompletableFuture.runAsync(() -> {
            JsonArray localModels = DBUtil.getLLMsByKey(Nvidia_AI_KEY);

            assert modelsCombobox != null;
            JsonObject apiKeyItem = OtherUtil.weightedRandomTarget(thisApiKeys);
            JsonArray models = ShineAIUtil.getAIModels(Nvidia_AI_KEY,Nvidia_AI_LLM_API,apiKeyItem);
            ...
            if (!models.isEmpty()) DBUtil.setLLMsByKey(Nvidia_AI_KEY,models);
            modelsCombobox.setModel(modelsToComboBoxModel());
