...
public class OpenAISettingPanel implements Configurable, Disposable {
    ...
    public void reset() {
        ...
        JsonObject settingInfo = state.getAISettingInfo(OpenAI_CONTENT_NAME);
    ...
    public void apply() {
        ...
        state.setAISettingInfo(OpenAI_CONTENT_NAME,setInfo);
    ...
    public void updateModels() {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            modelsCombobox.setModel(modelsToComboBoxModel());
            JsonElement currentModel = stateStore.getAISettingInfoByKey(OpenAI_CONTENT_NAME,"aiModel");
    ...
    private void addNewApikey(JComponent _component) {
        ...
        apikeyList.add(new AIApikeyComponent(apikeyInfo,thisApiKeys,_component,OpenAI_CONTENT_NAME,false),0);
        thisApiKeys.add(0,apikeyInfo);
        // 这里需要写入state
        stateStore.setAISettingInfoByKey(OpenAI_CONTENT_NAME,"apiKeys", JsonUtil.getJsonArray(thisApiKeys));
        updateLayout();
        scrollTop();
    }
    ...
    public void initApiKeysPanel() {
        if (!thisApiKeys.isEmpty()) {
            for (JsonObject item : thisApiKeys) {
                AIApikeyComponent apiKeyItem = new AIApikeyComponent(item,thisApiKeys,apikeyList,OpenAI_CONTENT_NAME,false);
